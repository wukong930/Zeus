  ...
  return (
    <div className="px-8 py-6 max-w-5xl space-y-5 animate-fade-in">
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-4">
        <MetricTile label="计划数" value={String(TRADE_PLANS.length)} caption="actionable" icon={Activity} tone="cyan" />
        <MetricTile label="平均置信度" value={`${Math.round(avgConfidence * 100)}%`} caption="model score" icon={Gauge} tone="up" />
        <MetricTile label="平均 R:R" value={`1:${avgRiskReward.toFixed(2)}`} caption="reward/risk" icon={Target} tone="warning" />
        <MetricTile label="平均保证金" value={`${avgMargin.toFixed(1)}%`} caption="margin usage" icon={WalletCards} tone={avgMargin > 25 ? "warning" : "violet"} />
      </div>

"use client";

import { createContext, useContext, useEffect, useMemo, useState, type ReactNode } from "react";
import { Languages } from "lucide-react";
import { cn } from "@/lib/utils";

export type Language = "zh" | "en";

interface I18nContextValue {
  lang: Language;
  setLang: (lang: Language) => void;
  toggleLang: () => void;
  text: (source: string) => string;
}

const STORAGE_KEY = "zeus-language";

const I18nContext = createContext<I18nContextValue | null>(null);

const EN: Record<string, string> = {
  "命令中心": "Command Center",
  "预警": "Alerts",
  "交易计划": "Trade Plans",
  "持仓地图": "Portfolio Map",
  "因果网络": "Causal Web",
  "新闻事件": "News Events",
  "事件智能引擎": "Event Intelligence",
  "事件智能": "Event Intelligence",
  "事件智能源事件": "Event intelligence source",
  "事件智能影响假设": "Event intelligence impact hypothesis",
  "源事件": "source events",
  "路径聚焦中": "Path focus active",
  "聚焦此路径": "Focus path",
  "取消路径聚焦": "Clear path focus",
  "显示完整因果网": "Show full causal graph",
  "支持证据": "Supporting Evidence",
  "反证线索": "Counter-Evidence",
  "暂无结构化证据": "No structured evidence yet",
  "当前事件智能节点暂无可展示的关联链路。": "No linked event intelligence path is available for this node.",
  "该源事件已被组织为可审计的商品影响链。先看证据摘要，再沿下游链路检查机制和市场出口。":
    "This source event has been organized into auditable commodity impact links. Start with the evidence summary, then follow downstream paths for mechanisms and market exits.",
  "该影响假设承接一个源事件，并把机制、方向和置信度投射到具体商品或区域。":
    "This impact hypothesis inherits a source event and projects mechanism, direction, and confidence onto a specific commodity or region.",
  "产业透镜": "Industry Lens",
  "板块": "Sectors",
  "未来实验室": "Future Lab",
  "策略锻造": "Strategy Forge",
  "笔记本": "Notebook",
  "分析": "Analytics",
  "设置": "Settings",
  "交易胜于未始": "Trades are won before they begin",
  "导航": "Navigation",
  "关闭": "Close",
  "跳转": "Navigation",
  "品种": "Symbols",
  "机制": "Mechanism",
  "区域": "Region",
  "筛选": "Filters",
  "合约同步中": "Syncing contracts",
  "合约接口暂不可用": "Contracts API is unavailable",
  "暂无合约数据": "No contract data",
  "操作": "Actions",
  "询问 AI Companion": "Ask AI Companion",
  "创建笔记": "Create Note",
  "搜索品种 / 跳转 / 执行...": "Search symbols / navigate / run...",
  "没找到匹配项": "No matches",
  "命令面板": "Command Palette",
  "打开导航": "Open navigation",
  "关闭导航": "Close navigation",
  "展开侧边栏": "Expand sidebar",
  "收起侧边栏": "Collapse sidebar",
  "数据": "Data",
  "活跃": "Active",
  "漂移": "Drift",
  "校准": "Calibration",
  "状态": "Regime",
  "正常": "Normal",
  "震荡 · 高波动": "Range · High Vol",
  "震荡 · 低波动": "Range · Low Vol",
  "趋势上行 · 低波动": "Trend Up · Low Vol",
  "趋势下行 · 低波动": "Trend Down · Low Vol",
  "运行态总览": "Runtime Overview",
  "开放持仓": "Open Positions",
  "暂无重点事件": "No focus event",
  "校准加载中": "Loading calibration",
  "校准接口暂不可用": "Calibration API is unavailable",
  "重点：": "Focus:",
  "展开全图": "Open Full Map",
  "实时活跃因果链": "Live causal chain",
  "当日预警流": "Today Alerts",
  "查看全部": "View All",
  "持仓概览": "Portfolio Overview",
  "个持仓": "positions",
  "总浮动盈亏": "floating PnL",
  "详情": "Details",
  "多": "Long",
  "空": "Short",
  "手": "lots",
  "板块热力图": "Sector Heatmap",
  "橙点 = 信号活跃": "Orange dot = active signal",
  "运行态因子": "Runtime Factors",
  "行情覆盖": "Market Coverage",
  "信号活跃": "Signal Activity",
  "方向强度": "Direction Strength",
  "内部一致": "Internal Alignment",
  "活跃信号": "Active Signals",
  "实时": "Live",
  "最新": "Latest",
  "运行态图谱": "Runtime graph",
  "接口返回": "API result",
  "过去 24h": "Last 24h",
  "本月预警": "Monthly Alerts",
  "最新预警": "Latest Alerts",
  "vs 上月": "vs last month",
  "校准进度": "Calibration",
  "校准样本": "Calibration Samples",
  "校准桶": "Calibration Buckets",
  "先验主导": "Prior Dominant",
  "平均权重": "Avg Weight",
  "信号权重校准": "Signal Weight Calibration",
  "按 signal / category / regime 展示当前权重、样本量、置信带和先验主导状态":
    "Current weights, samples, confidence bands, and prior dominance by signal / category / regime",
  "阈值可靠性分箱": "Threshold Reliability Bins",
  "按置信度分箱展示真实样本、命中率和校准偏差":
    "Resolved samples, hit rate, and calibration gap by confidence bin",
  "当前权重": "Current Weight",
  "基准": "base",
  "90% 置信带": "90% Confidence Band",
  "暂无信号权重校准样本": "No signal weight calibration samples yet",
  "mature": "mature",
  "decay": "decay",
  "suggested": "suggested",
  "active weight": "active weight",
  "candidate": "candidate",
  "prior dominant": "prior dominant",
  "No resolved calibration buckets are available yet.": "No resolved calibration buckets are available yet.",
  "Some rows are recent resolved-signal candidates and are not active weights yet.":
    "Some rows are recent resolved-signal candidates and are not active weights yet.",
  "样本量": "samples",
  "LLM 月度成本": "LLM Monthly Cost",
  "商品报价": "Market Quotes",
  "最新行情快照": "Latest market snapshot",
  "行情加载中": "Loading quotes",
  "行情暂不可用": "Quotes unavailable",
  "暂无行情": "No quote",
  "接口暂不可用": "API unavailable",
  "预算": "Budget",
  "黑色": "Ferrous",
  "橡胶": "Rubber",
  "能化": "Energy/Chem",
  "有色": "Base Metals",
  "农产": "Agriculture",
  "贵金属": "Precious Metals",
  "螺纹钢": "Rebar",
  "热卷": "Hot-rolled Coil",
  "铁矿石": "Iron Ore",
  "焦炭": "Coke",
  "焦煤": "Coking Coal",
  "天然橡胶": "Natural Rubber",
  "20号胶": "No.20 Rubber",
  "顺丁橡胶": "BR Rubber",
  "原油": "Crude Oil",
  "甲醇": "Methanol",
  "聚丙烯": "Polypropylene",
  "铜": "Copper",
  "铝": "Aluminum",
  "锌": "Zinc",
  "镍": "Nickel",
  "豆粕": "Soymeal",
  "豆油": "Soybean Oil",
  "棕榈油": "Palm Oil",
  "黄金": "Gold",
  "白银": "Silver",
  "系统配置 · LLM 供应商 · 通知渠道": "System configuration · LLM providers · notification channels",
  "数据源": "Data Sources",
  "调度任务": "Scheduler Jobs",
  "就绪": "Ready",
  "异常": "degraded",
  "注意": "Attention",
  "关注": "Watch",
  "严重": "Severe",
  "需要关注": "Needs attention",
  "无漂移数据": "No Drift Data",
  "严重 Drift 告警": "Severe Drift Alert",
  "Drift 关注提示": "Drift Watch Notice",
  "Drift 正常": "Drift Normal",
  "暂无 Drift 数据": "No Drift Data",
  "市场结构已明显偏离校准样本，需人工复核信号权重和阈值表现。":
    "Market structure has moved away from calibration samples. Human review of signal weights and threshold behavior is required.",
  "部分指标出现黄色漂移，建议观察下一轮调度结果，不自动修改生产阈值。":
    "Some indicators show yellow drift. Watch the next scheduler run; production thresholds remain unchanged.",
  "当前市场结构与校准样本保持一致，无需通知。":
    "Current market structure remains aligned with calibration samples. No notification is needed.",
  "调度器尚未写入 Drift 指标，等待下一轮监控任务。":
    "The scheduler has not written Drift metrics yet. Waiting for the next monitoring run.",
  "通知链路": "Notification Path",
  "重点指标": "Focus Metrics",
  "暂无需要通知的 Drift 指标": "No Drift metrics need notification",
  "顶栏": "Heartbeat",
  "打开 Analytics / Drift 监控查看红色指标": "Open Analytics / Drift to inspect red metrics",
  "复核相关 signal_type / category 的近期命中率": "Review recent hit rate by signal_type / category",
  "在人工审批前保持生产阈值不变": "Keep production thresholds unchanged until human approval",
  "观察黄色指标是否连续出现": "Watch whether yellow metrics persist",
  "检查数据源新鲜度和样本量": "Check data freshness and sample size",
  "必要时进入治理队列再调整校准参数": "Use governance review before changing calibration parameters",
  "保持监控，不自动修改生产阈值": "Keep monitoring; do not automatically change production thresholds",
  "确认 drift-monitor 调度任务是否启用": "Confirm the drift-monitor scheduler job is enabled",
  "notify": "notify",
  "observe": "observe",
  "observe_only": "observe only",
  "analytics_drift": "Analytics / Drift",
  "notification_settings": "Notification Settings",
  "不可用": "Unavailable",
  "接口降级": "API Degraded",
  "调度降级": "Scheduler Degraded",
  "健康": "Healthy",
  "活跃供应商": "active providers",
  "每日上限": "daily cap",
  "数据源健康": "Data Source Health",
  "免费源 / 注册源配置状态": "Free and registered source status",
  "调度运行态": "Scheduler Runtime",
  "最近任务状态与未配置项": "Recent job states and unconfigured items",
  "Handler 覆盖": "Handler Coverage",
  "规划中任务": "Planned Jobs",
  "未接 Handler": "Handler Missing",
  "未启用": "Disabled",
  "成功": "Success",
  "停用": "Disabled",
  "缺少 Key": "Missing key",
  "缺少依赖": "Missing dependency",
  "未配置": "Unconfigured",
  "规划中": "Planned",
  "失败": "Failed",
  "LLM 供应商": "LLM Providers",
  "Alert Agent 用量": "Alert Agent Usage",
  "本月调用": "monthly calls",
  "调用": "calls",
  "缓存命中": "cache hits",
  "暂无用量数据": "No usage data",
  "等待同步": "Waiting for sync",
  "等待后端用量同步": "Waiting for backend usage sync",
  "主路由": "Primary route",
  "候选": "Candidate",
  "路由状态": "Routing status",
  "数据库配置": "Database config",
  "环境变量配置": "Environment config",
  "未配置 Key": "Missing key",
  "LLM 供应商配置接口暂不可用": "LLM provider config API is unavailable",
  "多供应商支持，按场景路由": "Multi-provider routing by scenario",
  "主力": "Primary",
  "本月成本 / 预算": "Monthly cost / budget",
  "配置": "Configure",
  "通知渠道": "Notification Channels",
  "前端实时推送与外部 Webhook": "Realtime frontend push and external webhooks",
  "实时 SSE 推送（前端）": "Realtime SSE Push (Frontend)",
  "飞书 Webhook": "Feishu Webhook",
  "Email 通知": "Email Notification",
  "自定义 Webhook": "Custom Webhook",
  "预警去重设置": "Alert Deduplication",
  "控制重复预警与升级重发": "Control duplicates and severity upgrade resends",
  "对抗引擎 Warmup": "Adversarial Warmup",
  "人工覆盖历史组合检验是否只记录不压制信号":
    "Manually control whether historical combo checks are audit-only",
  "Warmup 模式（历史组合只记录）": "Warmup mode (historical combo audit-only)",
  "历史组合检验失败只进入审计记录，不会压制信号。":
    "Historical combo failures are recorded for audit only and do not suppress signals.",
  "样本充足的历史组合检验失败可进入 enforcing，并可能压制信号。":
    "Historical combo failures with enough samples can become enforcing and may suppress signals.",
  "当前模式": "Current Mode",
  "历史组合检验": "Historical Combo Check",
  "审计状态": "Audit State",
  "只记录": "Audit only",
  "样本充足后执行": "Enforce with enough samples",
  "只告警": "Alert only",
  "可压制": "May suppress",
  "同品种同方向 N 小时内只发一次": "Same symbol and direction only once per N hours",
  "同信号组合 N 小时内只发一次": "Same signal bundle only once per N hours",
  "每日预警上限": "Daily alert limit",
  "允许严重度升级时重新发送": "Resend when severity upgrades",
  "12 小时": "12 hours",
  "24 小时": "24 hours",
  "50 条": "50 alerts",
  "是": "Yes",
  "否": "No",
  "运行态配置": "Runtime Config",
  "配置同步中": "Syncing Config",
  "关于 Zeus": "About Zeus",
  "运行态版本": "Runtime Build",
  "当前版本已接入 Python 后端、事件总线、真实免费数据源、信号检测、校准循环、对抗引擎和 Alert Agent；接口不可用时会显示空态或降级状态，避免模拟数据冒充运行态结果。":
    "This build is connected to the Python backend, event bus, real free data sources, signal detection, calibration loop, adversarial engine, and Alert Agent. Unavailable APIs now render empty or degraded states instead of presenting mock data as runtime results.",
  "成本链加载中": "Loading cost chain",
  "成本链接口暂不可用": "Cost chain API is unavailable",
  "正在同步产业成本链、历史快照和质量评估。":
    "Syncing industrial cost chain, historical snapshots, and quality assessment.",
  "当前不再展示模拟成本链，请等待后端成本模型恢复后再查看分位成本、信号触发和调价模拟。":
    "Mock cost chains are no longer shown; wait for the backend cost model before viewing quantiles, signal triggers, and price simulations.",
  "当前板块": "Current Sector",
  "质量评估加载中": "Loading quality assessment",
  "质量评估接口暂不可用": "Quality assessment API is unavailable",
  "正在同步公开基准误差、历史场景触发和付费数据源决策。":
    "Syncing public benchmark drift, historical scenario hits, and paid-source decisions.",
  "成本链仍可查看，但本次没有质量评估结果；页面会明确显示质量评估不可用状态。":
    "The cost chain is still visible, but no quality assessment is available; the page now clearly marks the assessment as unavailable.",
  "暂无真实历史成本快照": "No real historical cost snapshots yet",
  "最新利润率": "Latest Margin",
  "实时观察事件间的因果传导。点击节点追溯上游 / 预测下游。":
    "Observe causal propagation between events in realtime. Click nodes to trace upstream and forecast downstream.",
  "事件": "Event",
  "信号": "Signal",
  "指标": "Metric",
  "反证": "Counter",
  "事件源": "Event Source",
  "假设生成": "Hypothesis",
  "证据校验": "Validation",
  "市场/预警": "Market/Alert",
  "新闻、天气和外部冲击": "News, weather, and external shocks",
  "产业逻辑与方向假设": "Industrial logic and directional hypotheses",
  "价格、持仓与基本面验证": "Price, positioning, and fundamental validation",
  "可交易影响与告警出口": "Tradable impact and alert outlet",
  "地缘": "Geopolitics",
  "持仓": "Positioning",
  "全量因果网：适合检查跨板块传导、孤立节点和链路完整性。":
    "Full causal network for cross-sector propagation, isolated nodes, and path integrity.",
  "持仓视角：聚焦会影响当前风险暴露的 NR/RU 与黑色链路。":
    "Portfolio view focused on NR/RU and ferrous paths affecting current exposure.",
  "反证视角：突出会压低置信度、阻止误触发的验证节点。":
    "Counter view highlights validation nodes that reduce confidence and prevent false triggers.",
  "预警追踪：只强化能够进入告警或人工审查的关键路径。":
    "Alert trace emphasizes paths entering alerts or manual review.",
  "核心视图：优先展示影响力、持仓、预警和高连接节点，其余按阶段聚合。":
    "Core view prioritizes influence, positions, alerts, and connected nodes; the rest are clustered by stage.",
  "完整视图：展示当前过滤下的所有节点，并按阶段重新排布。":
    "Expanded view shows every node in the current filter and relayouts them by stage.",
  "该阶段还有更多节点，切换完整视图后展开。":
    "This stage has more nodes. Switch to Expanded to reveal them.",
  "上游": "upstream",
  "下游": "downstream",
  "聚合": "Cluster",
  "更多节点": "more nodes",
  "源": "sources",
  "点击展开": "Click to expand",
  "事件池": "Event Pool",
  "默认展示优先级最高的事件，可手动加入图谱。":
    "The highest-priority events are shown by default; add more to the graph manually.",
  "当前视图暂无节点": "No nodes in this view",
  "暂无运行态因果图谱": "No Runtime Causal Graph",
  "因果图谱加载中": "Loading causal graph",
  "因果图谱接口暂不可用": "Causal graph API is unavailable",
  "当前暂无运行态因果图谱": "No runtime causal graph is available right now",
  "当前事件源暂无持仓下游": "No portfolio downstream for the selected event source",
  "当前事件源暂无反证下游": "No counter-evidence downstream for the selected event source",
  "当前事件源暂无预警下游": "No alert downstream for the selected event source",
  "当前事件源暂无下游节点": "No downstream nodes for the selected event source",
  "此视图现在只显示当前事件源直接或间接传导到的下游节点。":
    "This view only shows nodes directly or indirectly downstream of the selected event source.",
  "事件源作用域：仅展示当前事件源直接或间接传导到的下游节点。":
    "Event-source scope: only nodes directly or indirectly downstream of the selected event source are shown.",
  "当前样本暂无反证节点，已建议改用证据/反证视图。":
    "There are no counter-evidence nodes in the current sample. Use the evidence/counter view fallback.",
  "请调整筛选或选择更多事件。": "Adjust filters or select more events.",
  "全量": "All",
  "，": ", ",
  "。": ".",
  "Nodes": "Nodes",
  "来源": "Source",
  "回退": "Fallback",
  "同步中": "Syncing",
  "同步": "Sync",
  "实时数据": "Live Data",
  "部分降级": "Partial Degrade",
  "降级": "Degraded",
  "接口不可用": "API Unavailable",
  "运行态数据": "Runtime Data",
  "运行态": "Runtime",
  "正在从后端同步数据": "Syncing data from backend services.",
  "当前视图来自后端实时接口": "This view is backed by live backend APIs.",
  "当前视图来自运行态图谱接口": "This view is backed by the runtime graph API.",
  "部分接口不可用，视图混合实时数据和降级结果":
    "Some APIs are unavailable; this view mixes live data with degraded results.",
  "后端接口暂不可用，当前视图显示空态或不可用状态":
    "Backend APIs are unavailable; this view is showing an empty or unavailable state.",
  "世界风险地图": "World Risk Map",
  "打开世界风险地图": "Open World Risk Map",
  "核对证据链": "Verify Evidence Chain",
  "地图": "Map",
  "将新闻、天气、市场和人工事件组织成商品影响链。":
    "Organize news, weather, market, and manual events into commodity impact chains.",
  "搜索事件 / 品种 / 机制": "Search event / symbol / mechanism",
  "影响链": "Impact Chains",
  "事件链": "Event Chain",
  "事件智能链": "Event Intelligence Chain",
  "从新闻生成可审计的商品影响链": "Generate auditable commodity impact links from news",
  "影响分": "Impact Score",
  "更新时间": "Updated",
  "当前事件暂无影响链明细": "No impact-link details for this event yet",
  "查看事件智能链": "View Event Intelligence Chain",
  "当前新闻尚未生成事件智能链，可一键解析人物、地区、商品和影响机制。":
    "This news event has no intelligence chain yet. Generate one to parse people, regions, commodities, and impact mechanisms.",
  "事件智能生成失败": "Event intelligence generation failed",
  "生成中": "Generating",
  "生成事件智能链": "Generate Event Intelligence Chain",
  "关联品种": "Linked Symbols",
  "人工复核": "Human Review",
  "最高影响": "Max Impact",
  "质量通过": "Quality Pass",
  "质量评分": "Quality Score",
  "质量阻断": "Blocked",
  "质量复核": "Quality Review",
  "影子可用": "Shadow Ready",
  "决策级": "Decision Grade",
  "质量门": "Quality Gate",
  "事件质量门": "Event Quality Gate",
  "未评估": "Not Evaluated",
  "当前区域事件智能尚未通过质量门，地图只用于阅读复核，不放大自动风险。":
    "Event intelligence in this region has not passed the quality gate; the map keeps it for review without amplifying automated risk.",
  "阻断": "Blocked",
  "复核": "Review",
  "已通过": "Passed",
  "证据、来源、作用域和影响链完整性":
    "Evidence, source, scope, and impact-link integrity",
  "当前事件通过质量门，暂无阻断项。":
    "This event passes the quality gate with no blocking issues.",
  "事件已被拒绝，不能进入后续链路。": "This event was rejected and cannot enter downstream paths.",
  "事件缺少影响品种。": "The event is missing affected symbols.",
  "事件缺少影响机制。": "The event is missing impact mechanisms.",
  "事件缺少支持证据。": "The event is missing supporting evidence.",
  "事件暂无商品影响链。": "The event has no commodity impact links yet.",
  "事件影响链全部未通过质量门。": "All impact links failed the quality gate.",
  "事件影响链需要复核后才能升级。": "Impact links need review before upgrade.",
  "事件需要人工复核后才能升级。": "The event requires human review before upgrade.",
  "事件置信度偏低。": "Event confidence is low.",
  "事件来源可信度偏低。": "Source reliability is low.",
  "事件新鲜度不足。": "Event freshness is insufficient.",
  "高影响事件缺少反证线索。": "High-impact event is missing counter-evidence.",
  "影响链已被拒绝。": "Impact link was rejected.",
  "影响链缺少品种。": "Impact link is missing a symbol.",
  "影响链缺少机制。": "Impact link is missing a mechanism.",
  "影响链缺少支持证据。": "Impact link is missing supporting evidence.",
  "影响链置信度偏低。": "Impact link confidence is low.",
  "影响链当前仅建议观察。": "Impact link currently recommends watch only.",
  "影响链缺少机制说明。": "Impact link is missing rationale.",
  "影响": "Impact",
  "来源可信": "Source Reliability",
  "新鲜度": "Freshness",
  "事件队列": "Event Queue",
  "按影响分排序": "ranked by impact score",
  "事件智能接口暂不可用": "Event intelligence API is unavailable",
  "暂无事件智能结果": "No event intelligence results",
  "请选择事件": "Select an event",
  "需要人工确认": "Human confirmation required",
  "确认": "Confirm",
  "转人工复核": "Manual Review",
  "处理中": "Processing",
  "条候选链路": "candidate links",
  "语义假设": "Semantic Hypotheses",
  "条 LLM 候选影响": "LLM candidate impacts",
  "等待语义假设说明": "Waiting for semantic hypothesis rationale",
  "当前事件暂无商品影响链": "No commodity impact chain for this event",
  "修改链路": "Edit Link",
  "人工修改影响链": "Manual Impact Link Edit",
  "修改后回到人工复核，不改变生产阈值。":
    "Edits return the chain to human review and do not change production thresholds.",
  "没有可保存的链路修改": "No link changes to save",
  "品种、周期和机制说明不能为空": "Symbol, horizon, and rationale are required",
  "保存中": "Saving",
  "保存修改": "Save Changes",
  "取消": "Cancel",
  "机制说明": "Rationale",
  "编辑备注": "Edit Note",
  "说明本次修改原因": "Explain the reason for this edit",
  "每行一条证据": "One evidence item per line",
  "每行一条反证": "One counter-evidence item per line",
  "影响链修改失败": "Impact link edit failed",
  "治理时间线": "Governance Timeline",
  "确认、复核、语义增强和影响链修改留痕":
    "Trace confirmations, reviews, semantic enhancement, and impact-link edits",
  "操作者": "Operator",
  "正在加载治理历史": "Loading governance history",
  "治理历史暂不可用": "Governance history is unavailable",
  "暂无治理历史": "No governance history yet",
  "字段": "Field",
  "条语义假设": "semantic hypotheses",
  "生产影响": "Production Effect",
  "无生产影响": "No Production Effect",
  "规则解析生成": "Rule Resolver Created",
  "语义增强": "Semantic Enhancement",
  "进入治理队列": "Queued for Governance",
  "确认通过": "Confirmed by Human",
  "人工拒绝": "Human Rejected",
  "回到影子复核": "Returned to Shadow Review",
  "影响链修改": "Impact Link Edited",
  "治理队列": "Governance Queue",
  "统一审核自动学习、事件智能和校准变更。":
    "Unified review for automated learning, event intelligence, and calibration changes.",
  "复核队列": "Review Queue",
  "按状态、来源和目标筛选待审建议": "Filter queued proposals by status, source, and target",
  "搜索来源 / 目标 / 变更内容": "Search source / target / change content",
  "注意力分流": "Attention Triage",
  "全部分流": "All Triage",
  "必须复核": "Must Review",
  "影子观察": "Shadow Observe",
  "证据归档": "Evidence Archive",
  "平均注意力": "Avg Attention",
  "分流层级": "Triage Tier",
  "注意力分": "Attention Score",
  "人工处理": "Human Handling",
  "需要人工": "Human Required",
  "不打扰人工": "No Human Interrupt",
  "未分流": "Untriaged",
  "正在加载治理队列": "Loading governance queue",
  "暂无治理队列项": "No governance queue items",
  "请选择一个治理队列项": "Select a governance queue item",
  "审核结论只进入治理记录；生产写入由目标模块专用流程执行。":
    "Review decisions are recorded in governance; production writes stay in target-specific flows.",
  "已处理": "Processed",
  "待复核": "Pending Review",
  "已批准": "Approved",
  "已驳回": "Rejected",
  "已审查": "Reviewed",
  "批准": "Approve",
  "驳回": "Reject",
  "转影子复核": "Shadow Review",
  "标记已审查": "Mark Reviewed",
  "入队原因": "Queue Reason",
  "复核原因": "Review Reasons",
  "确认可执行": "Approve for Execution",
  "复核通过中": "Approving",
  "驳回计划": "Reject Plan",
  "交易计划复核失败": "Trade plan review failed",
  "交易计划采纳失败": "Trade plan adoption failed",
  "采纳中": "Adopting",
  "Alert Agent 标记需要人工复核": "Alert Agent requires manual review",
  "预警状态为 pending，不能直接采纳": "Alert status is pending, so the plan cannot be adopted directly",
  "预警状态为 suppressed，不能直接采纳": "Alert status is suppressed, so the plan cannot be adopted directly",
  "置信度处于确认档": "Confidence is in the confirmation tier",
  "置信度处于通知档": "Confidence is in the notification tier",
  "通知档信号被治理规则升级为复核": "Governance upgraded this notification-tier signal to review",
  "自动档信号仍被治理规则复核": "Governance still requires review for this auto-tier signal",
  "LLM 仲裁参与，需人工确认结论": "LLM arbitration was involved; human confirmation is required",
  "存在方向冲突": "Directional conflict detected",
  "置信度处于模糊区间": "Confidence is in an ambiguous range",
  "缺少该信号/品类/行情状态的历史校准样本":
    "Missing calibration history for this signal/category/regime",
  "跨板块影响链较长": "Cross-sector impact chain is long",
  "历史反馈提示谨慎": "Historical feedback suggests caution",
  "变更摘要": "Change Summary",
  "治理动作": "Governance Actions",
  "复核人": "Reviewer",
  "复核时间": "Review Time",
  "打开事件智能": "Open Event Intelligence",
  "候选影响链": "Candidate Impact Links",
  "原始治理载荷": "Raw Governance Payload",
  "目标键": "Target Key",
  "变更字段": "Changed Fields",
  "该队列项没有结构化摘要，请查看原始治理载荷。":
    "This queued item has no structured summary; inspect the raw governance payload.",
  "反馈学习": "Feedback Learning",
  "LLM 反思": "LLM Reflection",
  "实盘背离": "Live Divergence",
  "信号校准": "Signal Calibration",
  "事件智能对象": "Event Intelligence Item",
  "事件影响链": "Event Impact Link",
  "供给": "Supply",
  "单一来源": "Single Source",
  "低置信": "Low Confidence",
  "低来源可信度": "Low Source Reliability",
  "高影响不确定事件": "High-impact Uncertain Event",
  "影响链需要复核": "Impact Link Requires Review",
  "影子复核": "Shadow Review",
  "已确认": "Confirmed",
  "已拒绝": "Rejected",
  "利多": "Bullish",
  "利空": "Bearish",
  "混合": "Mixed",
  "风险情绪": "Risk Sentiment",
  "宏观": "Macro",
  "周期": "Horizon",
  "地理空间预警、天气异常与因果链路作用域":
    "Geospatial alerts, weather anomalies, and causal scopes",
  "覆盖区域": "Regions",
  "产区与链路": "Belts and corridors",
  "升温区域": "Elevated Regions",
  "风险分 ≥ 55": "Risk score ≥ 55",
  "最高风险": "Max Risk",
  "综合评分": "Composite score",
  "因果联动": "Causal Linkage",
  "真实运行态关联": "Runtime-linked",
  "全球风险地理层": "Global Risk Geography",
  "天气 baseline + 运行态预警 / 新闻 / 信号 / 持仓":
    "Weather baseline + runtime alerts / news / signals / positions",
  "天气 baseline + 运行态预警 / 新闻 / 信号 / 持仓 / 事件智能":
    "Weather baseline + runtime alerts / news / signals / positions / event intelligence",
  "图层等待同步": "Waiting for layers",
  "世界风险地图接口暂不可用": "World risk map API is unavailable",
  "世界风险地图加载中": "Loading world risk map",
  "当前暂无区域风险数据": "No regional risk data",
  "区域综合风险": "Regional Composite Risk",
  "区域情报档案": "Regional Dossier",
  "区域运行态": "Regional Runtime",
  "视觉图层": "Visual Layers",
  "渲染模式": "Render Mode",
  "轻量": "Light",
  "增强": "Enhanced",
  "SVG 主渲染": "SVG primary renderer",
  "WebGL 预备": "WebGL Ready",
  "当前仍由 SVG 承载交互，GPU 图层按 deck.gl 口径组织":
    "Interaction still runs on SVG while GPU layers are organized for deck.gl",
  "GPU 图层按 deck.gl 口径组织": "GPU layers are organized for deck.gl",
  "MapLibre底图": "MapLibre Basemap",
  "离线运行": "Offline",
  "视图同步": "View Sync",
  "已同步": "Synced",
  "图层联动": "Layer Sync",
  "增强阅读层": "Enhanced Reading Layer",
  "先读最高风险，再看跨区传导和证据密度":
    "Read the highest risk first, then cross-region transmission and evidence density",
  "主读区": "Primary Read",
  "链路总览": "Chain Overview",
  "风险动量": "Risk Momentum",
  "动量升温": "Momentum Rising",
  "动量降温": "Momentum Easing",
  "动量稳定": "Momentum Stable",
  "动量来源": "Momentum Driver",
  "触发时间": "Trigger Time",
  "跨区传导": "Cross-Region Transmission",
  "证据密度": "Evidence Density",
  "运行态证据": "Runtime Evidence",
  "证据健康": "Evidence Health",
  "来源可信度": "Source Reliability",
  "数据新鲜度": "Data Freshness",
  "新鲜来源": "Fresh Sources",
  "最高风险区域": "Highest Risk Region",
  "可追溯": "Traceable",
  "事件作用域": "Event Scope",
  "推荐动作": "Recommended Action",
  "来自世界风险地图": "From World Risk Map",
  "来自因果网络": "From Causal Web",
  "来自事件智能": "From Event Intelligence",
  "来自新闻事件": "From News Events",
  "已带入区域和品种上下文": "Region and symbol context is applied",
  "已按品种过滤事件智能结果": "Event intelligence results are filtered by symbol",
  "已按品种过滤新闻事件": "News events are filtered by symbol",
  "清除作用域": "Clear Scope",
  "当前事件作用域": "Current Event Scope",
  "地图已按这条事件智能链过滤，以下证据解释它为什么命中当前区域。":
    "The map is filtered by this event intelligence chain; the evidence below explains why it hit this region.",
  "当前区域未找到该事件智能链的直接证据，仅保留同品种 / 同机制观察。":
    "No direct evidence for this event intelligence chain was found in this region; same-symbol / same-mechanism watch is retained.",
  "命中证据": "Matched Evidence",
  "命中": "Matched",
  "未命中": "No Match",
  "先复核质量门": "Review Quality Gate First",
  "存在待复核或被阻断事件，先到事件智能确认质量，再追溯因果链。":
    "Review pending or blocked events in Event Intelligence before tracing the causal chain.",
  "先补证据": "Add Evidence First",
  "当前证据覆盖偏薄，优先查看新闻、天气或信号来源，再进入链路追溯。":
    "Evidence coverage is thin. Review news, weather, or signal sources before tracing the chain.",
  "打开新闻事件": "Open News Events",
  "打开因果网络": "Open Causal Web",
  "追溯升温链": "Trace Rising Chain",
  "风险正在升温且存在直接事件作用域，优先进入因果网络核对上下游传导。":
    "Risk momentum is rising with a direct event scope. Trace upstream and downstream propagation in Causal Web first.",
  "追溯因果链": "Trace Causal Chain",
  "当前区域已有可追溯事件作用域，可进入因果网络核对上下游。":
    "This region has a traceable event scope. Use Causal Web to verify upstream and downstream links.",
  "保持观察": "Keep Watching",
  "当前缺少直接事件作用域，先保持同商品观察，等待新鲜证据进入。":
    "No direct event scope is available yet. Keep same-commodity monitoring until fresh evidence arrives.",
  "当前区域证据覆盖偏薄，建议先阅读证据与反证，再判断是否需要进入因果网络。":
    "Evidence coverage is thin for this region. Read supporting and counter evidence before entering Causal Web.",
  "暂无支持证据": "No supporting evidence",
  "暂无反证": "No counter-evidence",
  "事件智能链路": "Event Intelligence Links",
  "事件智能证据": "Event Intelligence Evidence",
  "故事证据": "Story Evidence",
  "联动区域": "Linked Regions",
  "活跃图层": "Active Layers",
  "暂无传导": "No Transmission",
  "当前筛选下暂无跨区飞线": "No cross-region flows under the current filter",
  "共享合约": "Shared Symbol",
  "同商品": "Same Commodity",
  "暂无": "None",
  "MapLibre已就绪": "MapLibre Ready",
  "MapLibre异常": "MapLibre Error",
  "MapLibre加载中": "MapLibre Loading",
  "GeoJson区域": "GeoJson Regions",
  "Heatmap密度": "Heatmap Density",
  "Arc飞线": "Arc Flows",
  "Tile天气": "Weather Tiles",
  "瓦片": "Tiles",
  "瓦片准备中": "Preparing tiles",
  "天气瓦片": "Weather tiles",
  "风险瓦片": "Risk tiles",
  "轻负载": "Light load",
  "标准负载": "Normal load",
  "密集负载": "Dense load",
  "缓存": "Cache",
  "瓦片缓存命中": "Cache hit",
  "网络请求": "Network",
  "已刷新": "Refreshed",
  "已降载": "Throttled",
  "coarse": "coarse",
  "待接入": "Planned",
  "高风险索引": "Risk Index",
  "按当前筛选排序": "Sorted by current filter",
  "暂无区域": "No regions",
  "低风险": "Low Risk",
  "观察": "Watch",
  "升温": "Elevated",
  "高风险": "High Risk",
  "极高风险": "Critical Risk",
  "基线数据": "Baseline Data",
  "部分数据": "Partial Data",
  "天气异常": "Weather Anomaly",
  "降水距平": "Precipitation Anomaly",
  "降水分位": "Rainfall Percentile",
  "温度分位": "Temperature Percentile",
  "7日降水": "7D Rainfall",
  "当前气温": "Current Temp",
  "1小时降水": "1H Rainfall",
  "湿度": "Humidity",
  "风速": "Wind Speed",
  "洪涝风险": "Flood Risk",
  "主要驱动": "Primary Drivers",
  "打开关联因果网络": "Open Linked Causal Web",
  "当前区域暂无直接因果联动": "No direct causal linkage for this region",
  "按商品属性自适应解释天气、物流、供应和价格传导":
    "Adaptively explain weather, logistics, supply, and price transmission by commodity",
  "覆盖": "Coverage",
  "最高": "Max",
  "点击区域查看动态风险链": "Click a region to inspect its dynamic risk chain",
  "传导链由商品属性、天气 baseline、运行态预警、新闻、信号和持仓共同生成。":
    "The chain is generated from commodity attributes, weather baseline, runtime alerts, news, signals, and positions.",
  "动态预警": "Adaptive Alerts",
  "商品传导链": "Commodity Transmission Chain",
  "证据": "Evidence",
  "打开同商品因果网络": "Open Same-Commodity Causal Web",
  "气候": "Climate",
  "天气状态": "Weather Regime",
  "生产": "Production",
  "物流": "Logistics",
  "供应": "Supply",
  "需求": "Demand",
  "政策": "Policy",
  "价格": "Price",
  "密度": "Density",
  "热力": "Heat",
  "飞线": "Flows",
  "地图标签": "Labels",
  "全部": "All",
  "刷新": "Refresh",
  "自动": "Auto",
  "手动": "Manual",
  "实时轮询": "Live polling",
  "轮询中": "Polling",
  "手动刷新": "Manual refresh",
  "缩小": "Zoom out",
  "放大": "Zoom in",
  "重置视图": "Reset view",
  "新闻": "News",
  "Verified": "Verified",
  "Mode": "Mode",
  "View": "View",
  "Visible": "Visible",
  "Fit view": "Fit view",
  "Close": "Close",
  "Close node details": "Close node details",
  "Fresh": "Fresh",
  "Impact": "Impact",
  "State": "State",
  "Quiet": "Quiet",
  "Propagation": "Propagation",
  "Causal Layers": "Causal Layers",
  "Semantic Stages": "Semantic Stages",
  "Position": "Position",
  "Alert": "Alert",
  "All": "All",
  "all": "all",
  "Portfolio": "Portfolio",
  "Counter": "Counter",
  "Alert Trace": "Alert Trace",
  "live": "live",
  "replay": "replay",
  "explorer": "explorer",
  "bullish": "bullish",
  "bearish": "bearish",
  "neutral": "neutral",
  "verified": "verified",
  "unverified": "unverified",
  "RB 触发了 cost_support_pressure": "RB triggered cost_support_pressure",
  "螺纹钢主力": "Rebar Main",
  "20号胶主力": "No.20 Rubber Main",
  "沪铜主力": "SHFE Copper Main",
  "铁矿主力": "Iron Ore Main",
  "棕榈油主力": "Palm Oil Main",
  "原油主力": "Crude Oil Main",
  "螺纹跌破 P75 边际成本支撑": "Rebar broke below P75 marginal cost support",
  "泰国天胶产区遭遇暴雨": "Heavy rain hits Thai natural rubber regions",
  "沪伦比价 Z-score 突破 +2.1": "SHFE/LME copper ratio Z-score breaks +2.1",
  "铁矿 ATR 放大，趋势进入震荡": "Iron ore ATR expands, trend shifts to range",
  "马来棕榈油库存意外增加": "Malaysian palm oil inventory rises unexpectedly",
  "原油动量信号触发但置信度偏低": "Crude oil momentum signal triggered with low confidence",
  "螺纹现货价 4280 已触及高炉成本曲线 P75 分位（4310），连续 2 周持续。结合焦炭弱势 + 钢厂利润率转负 → 触发产能收缩预期。":
    "Rebar spot at 4,280 has reached the P75 blast-furnace cost curve level (4,310) for two straight weeks. Weak coke plus negative mill margins trigger capacity-cut expectations.",
  "泰国南部主产区（合艾、宋卡）出现持续强降雨，未来 7 天预报维持。历史 5 次类似事件 4 次推升 NR 短期上涨 3-7%。":
    "Sustained heavy rainfall hit key southern Thai producing areas (Hat Yai and Songkhla), with forecasts persisting for seven days. Four of five similar historical events lifted NR by 3-7% short term.",
  "近月沪伦比价偏离 90 日均值 2.1 个标准差，历史回归概率 0.78。结合人民币汇率边际转强 → 套利窗口出现。":
    "The near-month SHFE/LME copper ratio is 2.1 standard deviations above its 90-day mean, with a historical mean-reversion probability of 0.78. A firmer RMB opens an arbitrage window.",
  "铁矿过去 10 日 ATR 百分位升至 78，ADX 跌至 18，从 trend_down 切换至 range_high_vol。":
    "Iron ore's 10-day ATR percentile rose to 78 while ADX fell to 18, shifting from trend_down to range_high_vol.",
  "MPOB 月报显示库存环比 +5.8%，超预期 +2.1%。供给压力释放，价格短期承压。":
    "The MPOB monthly report showed inventory up 5.8% month over month, 2.1 points above expectations. Supply pressure weighs on near-term prices.",
  "5 日动量突破 +2.3%，但波动率扩大 + 历史命中率仅 0.52。建议观察 1-2 个交易日确认。":
    "Five-day momentum broke +2.3%, but volatility expanded and the historical hit rate is only 0.52. Watch one to two sessions for confirmation.",
  "做多": "Long",
  "做空": "Short",
  "止损": "Stop",
  "入场": "Entry",
  "当前": "Current",
  "目标": "Target",
  "保证金占用": "Margin Usage",
  "组合风险": "Portfolio Risk",
  "信号摘要": "Signal Summary",
  "采纳建议": "Accept",
  "可执行": "Actionable",
  "待确认": "Needs Review",
  "等待复核": "Awaiting Review",
  "修改": "Edit",
  "拒绝": "Reject",
  "成本支撑 + 库存低位 + 历史类比命中率 0.72": "Cost support + low inventory + 0.72 historical analogue hit rate",
  "产区暴雨 + 历史 4/5 命中 + 保税区报价跟涨": "Producing-region rain + 4/5 historical hits + bonded-zone quotes following higher",
  "沪伦比 Z=+2.1 + 历史回归概率 0.78": "SHFE/LME ratio Z=+2.1 + 0.78 historical reversion probability",
  "待人工确认": "Manual Review",
  "通过": "Approve",
  "已提交": "Submitted",
  "提交失败": "Submit failed",
  "触发新闻": "Triggering News",
  "反馈": "Feedback",
  "同意": "Agree",
  "不确定": "Uncertain",
  "不同意": "Disagree",
  "已记录": "Recorded",
  "稍后重试": "Retry later",
  "当前可见": "Visible Now",
  "已验证": "Verified",
  "搜索预警...": "Search alerts...",
  "严重度": "Severity",
  "预警流": "Alert Stream",
  "按严重度、板块和人工确认状态扫描当前事件。": "Scan current events by severity, sector, and manual review state.",
  "预警加载中": "Loading alerts",
  "没有匹配的预警": "No matching alerts",
  "暂无板块维度": "No sector dimensions",
  "无行情": "No quote",
  "Open AI Companion": "Open AI Companion",
  "Context aware research copilot": "Context-aware research copilot",
  "我是你的 Zeus 研究伙伴。我知道你当前在哪个页面、看哪个预警，可以帮你解释、对比、追问。":
    "I am your Zeus research partner. I know the current page and alert context, and can help explain, compare, and follow up.",
  "建议提问": "Suggested Questions",
  "为什么 RB 触发了 cost_support_pressure？": "Why did RB trigger cost_support_pressure?",
  "解释一下当前的 Drift 状态": "Explain the current drift state",
  "我的橡胶持仓有哪些下游风险？": "What downstream risks affect my rubber positions?",
  "对比上月和本月的命中率变化": "Compare hit-rate changes month over month",
  "你": "You",
  "思考中...": "Thinking...",
  "问点什么...": "Ask anything...",
  "上下文：当前页面 + 最近 3 条预警": "Context: current page + latest 3 alerts",
  "总数": "Total",
  "交叉验证": "Cross-Verified",
  "人工确认": "Manual Review",
  "品种 / 标题 / 摘要": "Symbol / title / summary",
  "新闻事件加载中": "Loading news events",
  "没有匹配的新闻事件": "No matching news events",
  "OPEC+ 宣布延长自愿减产，原油短线走强": "OPEC+ extends voluntary cuts, crude strengthens short term",
  "OPEC+ 延长自愿减产安排，市场重新评估二季度供应缺口，对 SC 原油形成偏多冲击。":
    "OPEC+ extended voluntary cuts. The market is reassessing Q2 supply gaps, creating a bullish shock for SC crude.",
  "泰国南部橡胶主产区连续强降雨": "Continuous heavy rain in southern Thai rubber regions",
  "合艾和宋卡降雨持续，割胶节奏受扰，NR/RU 近端供应预期收紧。":
    "Rainfall continues in Hat Yai and Songkhla, disrupting tapping and tightening near-end NR/RU supply expectations.",
  "交易所提示铁矿石合约交易风险": "Exchange warns of iron ore contract trading risk",
  "交易所发布铁矿石波动风险提示，单源高严重度事件等待人工确认后再进入评估。":
    "The exchange issued an iron ore volatility risk notice. This single-source high-severity event waits for manual confirmation before evaluation.",
  "类型": "Type",
  "时效": "Horizon",
  "来源数": "Sources",
  "置信度": "Confidence",
  "影响品种": "Affected Symbols",
  "质量门槛": "Quality Gates",
  "来源共识 / 严重度门槛 / 人工门槛": "Source quorum / severity gate / manual gate",
  "严重度 ≥ 3": "Severity >= 3",
  "跨源验证": "Cross-source",
  "无需确认": "No manual gate",
  "原文链接": "Original Link",
  "policy": "policy",
  "supply": "supply",
  "demand": "demand",
  "inventory": "inventory",
  "weather": "weather",
  "market": "market",
  "macro": "macro",
  "manual": "manual",
  "social": "social",
  "shipping": "shipping",
  "event_intelligence": "event intelligence",
  "impact_link": "impact link",
  "news_event": "news event",
  "spread_anomaly": "spread anomaly",
  "basis_shift": "basis shift",
  "momentum": "momentum",
  "regime_shift": "regime shift",
  "inventory_shock": "inventory shock",
  "event_driven": "event driven",
  "price_gap": "price gap",
  "rubber_supply_shock": "rubber supply shock",
  "capacity_contraction": "capacity contraction",
  "restart_expectation": "restart expectation",
  "median_pressure": "median pressure",
  "marginal_capacity_squeeze": "marginal capacity squeeze",
  "breaking": "breaking",
  "mixed": "mixed",
  "unclear": "unclear",
  "short": "short",
  "medium": "medium",
  "immediate": "immediate",
  "cross_verified": "Cross-verified",
  "single_source": "Single source",
  "ferrous": "Ferrous",
  "rubber": "Rubber",
  "energy": "Energy/Chem",
  "chemical": "Chemicals",
  "nonferrous": "Base Metals",
  "metals": "Base Metals",
  "agri": "Agriculture",
  "agriculture": "Agriculture",
  "precious": "Precious Metals",
  "precious_metals": "Precious Metals",
  "每条建议是一份完整的飞行计划——入场、止损、目标、保证金、组合风险一目了然。":
    "Each recommendation is a complete flight plan: entry, stop, target, margin, and portfolio risk at a glance.",
  "计划数": "Plans",
  "当前暂无待执行或待确认交易计划": "No actionable or review-pending trade plans right now",
  "平均置信度": "Avg Confidence",
  "平均保证金": "Avg Margin",
  "证据包摘要": "Evidence Packet",
  "支持要点": "Support",
  "上下文证据": "Context Evidence",
  "反证 / 待确认": "Counter / Checks",
  "计划类型": "Plan Type",
  "信号类型": "Signal Type",
  "预警状态": "Alert Status",
  "置信档": "Confidence Tier",
  "对抗运行态": "Adversarial Runtime",
  "历史样本": "Historical Samples",
  "支持信号": "Support Signals",
  "板块层方向判断 + 各品种活跃度 + conviction 因子":
    "Sector direction, symbol activity, and conviction factors",
  "板块数": "Sectors",
  "平均 conviction": "Avg Conviction",
  "方向状态": "Bias State",
  "橙色脉动 = 信号活跃 · 颜色亮度 = 涨跌幅":
    "Orange pulse = active signal · color intensity = price move",
  "核心因子（4 维 conviction）": "Core Factors (4D Conviction)",
  "成本": "Cost",
  "库存": "Inventory",
  "季节": "Seasonality",
  "利润": "Margin",
  "可视化展示持仓在传导图中的位置 + 组合风险":
    "Visualize where positions sit in the propagation map plus portfolio risk.",
  "组合 P&L": "Portfolio P&L",
  "添加持仓": "Add Position",
  "当日浮动": "Intraday Floating",
  "日 95% 置信": "day 95% confidence",
  "压力损失": "Stress Loss",
  "个场景": "scenarios",
  "持仓列表": "Position List",
  "笔持仓": "positions",
  "包含传导图激活范围": "includes propagation-map activation",
  "方向": "Direction",
  "手数": "Lots",
  "入场均价": "Avg Entry",
  "现价": "Last Price",
  "浮动盈亏": "Floating PnL",
  "收益率": "Return",
  "数据版本": "Data Vintage",
  "开仓日期": "Open Date",
  "减半": "Halve",
  "平仓": "Close",
  "持仓加载中": "Loading positions",
  "当前没有开放持仓": "No open positions",
  "传导图激活范围": "Propagation Activation",
  "持仓自动激活的关联品种监控": "Related-symbol monitoring activated by positions",
  "暂无开放持仓，传导图保持待机。": "No open positions. Propagation monitoring stays on standby.",
  "板块集中度提示": "Sector Concentration Notice",
  "当前保证金占用": "Current margin usage",
  "高相关或高占用持仓会在建议生成时降级。":
    "Highly correlated or high-usage positions are downgraded when recommendations are generated.",
  "激活监控品种": "Activated Symbols",
  "替代品": "Substitute",
  "下游替代": "Downstream substitute",
  "链路锚点": "Path anchor",
  "下游化工": "Downstream chemical",
  "上游原料": "Upstream input",
  "二级原料": "Secondary input",
  "Monte Carlo 价格路径模拟 + What-if 假设检验": "Monte Carlo price-path simulation + what-if hypothesis testing",
  "场景配置": "Scenario Setup",
  "目标链条": "Target Chain",
  "假设条件": "Assumptions",
  "模拟参数": "Simulation Parameters",
  "模拟次数": "Simulations",
  "时间跨度": "Time Horizon",
  " 天": " days",
  "正在推演": "Running",
  "运行推演": "Run Simulation",
  "概率扇": "Probability Fan",
  "推演完成": "Complete",
  "未来": "next",
  "天价格路径分布": "day price-path distribution",
  "次模拟": "simulations",
  "正在沿传导图模拟价格路径": "Simulating price paths along the propagation map",
  "P5 下行风险": "P5 Downside Risk",
  "5% 分位": "5% percentile",
  "P50 中位数路径": "P50 Median Path",
  "预期价格": "Expected price",
  "P95 上行": "P95 Upside",
  "95% 分位": "95% percentile",
  "情景叙事": "Scenario Narrative",
  "冲击": "Shock",
  "跌破基准概率": "break-below-base probability",
  "关键风险": "Key Risks",
  "建议动作": "Suggested Actions",
  "传导路径": "Propagation Paths",
  "滞后": "lag",
  "天": "days",
  "配置场景后运行推演": "Configure a scenario, then run the simulation",
  "运行态行情": "Runtime Market Data",
  "手动基准": "Manual Base",
  "静态基准": "Static Base",
  "场景基准价使用静态回退": "Scenario base price is using a static fallback",
  "未找到目标品种最新行情，推演仍可运行，但价格锚来自内置静态基准。":
    "Latest market data for the target symbol was not found. The simulation can still run, but its price anchor comes from a built-in static baseline.",
  "部分可用": "Partially Available",
  "20号胶 / 橡胶链": "No.20 Rubber / Rubber Chain",
  "螺纹钢 / 黑色链": "Rebar / Ferrous Chain",
  "原油 / 能化链": "Crude Oil / Energy-Chem Chain",
  "NR 原料冲击": "NR raw-material shock",
  "BR 替代品": "BR substitute",
  "RU 盘面冲击": "RU screen shock",
  "燃料油": "Fuel Oil",
  "乙二醇": "Ethylene Glycol",
  "策略研究、回测、参数调优 · vectorbt + DoWhy": "Strategy research, backtesting, and parameter tuning · vectorbt + DoWhy",
  "回测质量样本不足": "Insufficient Backtest Quality Samples",
  "Strategy Forge 当前只使用已完成建议的真实归因结果；缺少足够样本时不会展示硬编码回测表现。":
    "Strategy Forge now uses only real attribution outcomes from completed recommendations; when samples are insufficient, it does not show hard-coded backtest performance.",
  "样本": "samples",
  "暂无已完成建议样本": "No completed recommendation samples",
  "新建笔记": "New Note",
  "未接入": "Not Connected",
  "无本地示例数据": "No Local Sample Data",
  "暂无笔记": "No notes",
  "研究笔记暂未接入运行态存储": "Research notebook is not connected to runtime storage yet",
  "当前页面不再展示固定示例笔记；接入 Notebook API 后会显示用户保存的研究、假设和交易日志。":
    "This page no longer shows fixed sample notes. Once the Notebook API is connected, it will show saved research, hypotheses, and trade journals.",
  "当前状态": "Current State",
  "不会显示固定演示内容": "Fixed demo content is not shown",
  "暂无运行态笔记": "No runtime notes",
  "等待后端笔记存储接口接入。": "Waiting for the backend notebook storage API.",
  "暂无运行态关联": "No runtime links",
  "预警、假设和交易归因暂未生成真实引用关系。":
    "Alerts, hypotheses, and trade attribution have not produced real reference links yet.",
  "接入笔记 CRUD API 与持久化存储。": "Connect the notes CRUD API and persistent storage.",
  "将笔记与预警、假设和交易归因建立真实关联。":
    "Create real links between notes, alerts, hypotheses, and trade attribution.",
  "启用搜索、标签和自动保存后再开放新建入口。":
    "Enable search, tags, and autosave before opening note creation.",
  "只读": "Read Only",
  "笔记": "Notes",
  "搜索笔记 / 假设 / 预警": "Search notes / hypotheses / alerts",
  "运行态笔记": "Runtime Notes",
  "运行态研究报告、学习假设与预警引用。":
    "Runtime research reports, learning hypotheses, and alert references.",
  "预警引用": "Alert Refs",
  "文件夹": "Folders",
  "正在加载运行态笔记": "Loading runtime notes",
  "笔记本接口暂不可用": "Notebook API is unavailable",
  "没有匹配的运行态笔记": "No matching runtime notes",
  "研究报告": "Research Report",
  "学习假设": "Learning Hypothesis",
  "研究假设": "Research Hypothesis",
  "策略研究": "Strategy Research",
  "交易复盘": "Trade Review",
  "笔记状态": "Status",
  "更新": "Updated",
  "引用关系": "Reference Links",
  "无状态": "No Status",
  "报告": "Report",
  "请选择笔记": "Select a note",
  "标签": "Tags",
  "暂无标签": "No tags",
  "添加": "Add",
  "自动保存": "Autosaved",
  "关联": "Links",
  "下一步行动": "Next Actions",
  "关联预警 · 关联假设": "Linked Alerts · Linked Hypotheses",
  "假设": "Hypothesis",
  "命中率": "hit rate",
  "引用此笔记": "References to this note",
  "提及的假设": "Mentioned Hypotheses",
  "提及的预警": "Mentioned Alerts",
  "假设库": "Hypothesis Library",
  "交易日志": "Trade Journal",
  "冲突": "Conflict",
  "航运": "Shipping",
  "阈值": "Threshold",
  "化工": "Chemical",
  "传导": "Propagation",
  "现货": "Spot",
  "盘面": "Screen",
  "天气": "Weather",
  "产区": "Producing Area",
  "支撑": "Support",
};

const CJK_PATTERN = /[\u4e00-\u9fff]/;
const REVERSE_EN = Object.fromEntries(
  Object.entries(EN)
    .filter(([source]) => CJK_PATTERN.test(source))
    .map(([source, target]) => [target, source])
) as Record<string, string>;

const ZH: Record<string, string> = {
  ...REVERSE_EN,
  "notify": "通知",
  "observe": "观察",
  "observe_only": "仅观察",
  "heartbeat": "顶栏",
  "analytics_drift": "Analytics / Drift",
  "notification_settings": "通知渠道",
  "Warmup": "Warmup",
  "Enforcing": "强制执行",
  "Causal Web": "因果网络",
  "Trades are won before they begin": "交易胜于未始",
  "Futures Intel": "期货智能",
  "published": "已发布",
  "proposed": "已提出",
  "reviewed": "已审查",
  "shadow_testing": "影子测试",
  "validated": "已验证",
  "applied": "已应用",
  "rejected": "已拒绝",
  "open_directional": "单品种方向",
  "open_spread": "价差交易",
  "pending_review": "待确认",
  "confirm": "确认档",
  "auto": "自动",
  "warmup": "预热",
  "active": "活跃",
  "failed": "失败",
  "new": "新建",
  "medium_evidence": "中等证据",
  "weak_evidence": "弱证据",
  "strong_evidence": "强证据",
  "Data": "数据",
  "Nodes": "节点",
  "Node": "节点",
  "Live": "实时",
  "Replay": "回放",
  "Explorer": "探索",
  "Mode": "模式",
  "View": "视图",
  "Visible": "可见",
  "Fit view": "适配视图",
  "Close": "关闭",
  "Close node details": "关闭节点详情",
  "Fresh": "新鲜度",
  "Impact": "影响",
  "Quiet": "静默",
  "Propagation": "传导",
  "Causal Layers": "因果层",
  "Semantic Stages": "语义阶段",
  "Position": "持仓",
  "Alert": "预警",
  "All": "全量",
  "all": "全部",
  "Portfolio": "持仓",
  "Counter": "反证",
  "Alert Trace": "预警追踪",
  "Core": "核心",
  "Expanded": "完整",
  "Active": "活跃",
  "Verified": "已验证",
  "Calibration": "校准",
  "State": "状态",
  "live": "实时",
  "replay": "回放",
  "explorer": "探索",
  "bullish": "偏多",
  "bearish": "偏空",
  "neutral": "中性",
  "verified": "已验证",
  "unverified": "未验证",
  "blocker": "阻断",
  "warning": "警告",
  "info": "提示",
  "impact_link": "影响链路",
  "news_event": "新闻事件",
  "upstream": "上游",
  "downstream": "下游",
  "hit": "命中",
  "conf": "置信",
  "confidence": "置信度",
  "conviction": "信念强度",
  "Severity": "严重度",
  "Critical": "严重",
  "High": "高",
  "Medium": "中",
  "Low": "低",
  "CRITICAL": "严重",
  "HIGH": "高",
  "MEDIUM": "中",
  "LOW": "低",
  "critical": "严重",
  "high": "高",
  "medium": "中",
  "low": "低",
  "SYNC": "同步",
  "MOCK": "模拟",
  "requires focus": "需要关注",
  "manual gates": "人工门槛",
  "total": "总数",
  "source quorum": "来源共识",
  "manual gate": "人工门槛",
  "coarse": "粗",
  "轻负载": "轻负载",
  "标准负载": "标准负载",
  "密集负载": "密集负载",
  "缓存": "缓存",
  "瓦片缓存命中": "缓存命中",
  "网络请求": "网络请求",
  "已刷新": "已刷新",
  "已降载": "已降载",
  "cross_verified": "交叉验证",
  "single_source": "单一来源",
  "events": "事件",
  "policy": "政策",
  "supply": "供应",
  "demand": "需求",
  "inventory": "库存",
  "logistics": "物流",
  "cost": "成本",
  "risk_sentiment": "风险情绪",
  "geopolitical": "地缘",
  "macro": "宏观",
  "weather": "天气",
  "market": "行情",
  "manual": "人工",
  "social": "社媒",
  "shipping": "航运",
  "alert": "预警",
  "news": "新闻",
  "signal": "信号",
  "position": "持仓",
  "event_intelligence": "事件智能",
  "spread_anomaly": "价差异常",
  "basis_shift": "基差变动",
  "momentum": "动量信号",
  "regime_shift": "状态切换",
  "inventory_shock": "库存冲击",
  "event_driven": "事件驱动",
  "price_gap": "价格跳空",
  "rubber_supply_shock": "橡胶供应冲击",
  "capacity_contraction": "产能收缩",
  "restart_expectation": "复产预期",
  "median_pressure": "中位成本压力",
  "marginal_capacity_squeeze": "边际产能挤压",
  "baseline": "基线",
  "breaking": "突发",
  "mixed": "多空混合",
  "unclear": "不明确",
  "coverage": "覆盖",
  "orange pulse": "橙色脉动",
  "cross-sector": "跨板块",
  "sector bias": "板块偏向",
  "current": "当前",
  "resolved": "已归因",
  "queue": "队列",
  "flagged": "已标记",
  "review required": "需要复核",
  "review": "复核",
  "stable": "稳定",
  "no samples": "无样本",
  "resolved signals": "已归因信号",
  "waiting for resolved signals": "等待已归因信号",
  "Risk-on": "风险偏好",
  "Defensive": "防御",
  "chemical": "能化",
  "nonferrous": "有色",
  "agriculture": "农产",
  "precious_metals": "贵金属",
  "actionable": "可执行",
  "model score": "模型评分",
  "reward/risk": "收益/风险",
  "Risk": "风险",
  "Reward": "收益",
  "Adversarial": "对抗验证",
  "Time Machine": "时间机",
  "vintage": "版本",
  "margin usage": "保证金占用",
  "active providers": "活跃供应商",
  "realtime enabled": "实时已启用",
  "symbol direction": "品种方向",
  "daily cap": "每日上限",
  "active symbols": "活跃品种",
  "loading": "加载中",
  "decision-grade": "决策级",
  "diagnostic": "诊断级",
  "raw Sharpe gated": "原始夏普已门控",
  "max drawdown": "最大回撤",
  "Regime": "状态",
  "Win": "胜率",
  "Total Return": "总收益",
  "Path Risk": "路径风险",
  "PIT Universe": "PIT 品种池",
  "Multiple Testing": "多重检验",
  "decision-grade gate": "决策级门槛",
  "Regime Profile": "状态画像",
  "Path Metrics": "路径指标",
  "Origin Basket": "产区篮子",
  "Benchmark Drift": "基准偏离",
  "Historical Cases": "历史案例",
  "Purchase Decision": "采购决策",
  "Sources": "来源",
  "PASS": "通过",
  "REVIEW": "复核",
  "Underwater / pain / recovery / CVaR": "水下回撤 / 痛苦指数 / 修复 / CVaR",
  "Sharpe / hit rate / drawdown by market state": "按市场状态拆解夏普、命中率和回撤",
  "Walk-forward": "滚动前推",
  "VALID": "有效",
  "BLOCKED": "阻断",
  "Notes": "笔记",
  "Linked": "关联",
  "Yesterday": "昨天",
  "Open Full Map": "展开全图",
  "View All": "查看全部",
  "News Events": "新闻事件",
  "Trade Plans": "交易计划",
  "Portfolio Map": "持仓地图",
  "Sectors": "板块",
  "Future Lab": "未来实验室",
  "Strategy Forge": "策略锻造",
  "Settings": "设置",
  "Alerts": "预警",
  "Analytics": "分析",
  "系统运行健康度 + 个人交易归因": "系统运行健康度 + 个人交易归因",
  "推荐归因": "推荐归因",
  "Goal B 命脉": "Goal B 命脉",
  "校准仪表盘": "校准仪表盘",
  "反思假设": "反思假设",
  "Drift 监控": "Drift 监控",
  "closed": "已归因",
  "bucket": "分桶",
  "distribution": "分布",
  "胜率切片分析": "胜率切片分析",
  "按维度看哪些场景下系统表现最好": "按维度看哪些场景下系统表现最好",
  "按信号类型": "按信号类型",
  "按板块": "按板块",
  "证据": "证据",
  "变更建议": "变更建议",
  "仅记录假设": "仅记录假设",
  "base": "基准",
  "rolling": "滚动",
  "scenario simulation failed": "场景推演失败",
  "Monte Carlo 价格路径模拟 + What-if 假设检验": "Monte Carlo 价格路径模拟 + What-if 假设检验",
  "目标链条": "目标链条",
  "假设条件": "假设条件",
  "模拟参数": "模拟参数",
  "模拟次数": "模拟次数",
  "时间跨度": "时间跨度",
  "正在推演": "正在推演",
  "运行推演": "运行推演",
  "推演完成": "推演完成",
  "未来": "未来",
  "天价格路径分布": "天价格路径分布",
  "次模拟": "次模拟",
  "正在沿传导图模拟价格路径": "正在沿传导图模拟价格路径",
  "冲击": "冲击",
  "跌破基准概率": "跌破基准概率",
  "关键风险": "关键风险",
  "建议动作": "建议动作",
  "传导路径": "传导路径",
  "滞后": "滞后",
  "天": "天",
  " 天": " 天",
  "配置场景后运行推演": "配置场景后运行推演",
  "20号胶 / 橡胶链": "20号胶 / 橡胶链",
  "螺纹钢 / 黑色链": "螺纹钢 / 黑色链",
  "原油 / 能化链": "原油 / 能化链",
  "NR 原料冲击": "NR 原料冲击",
  "BR 替代品": "BR 替代品",
  "RU 盘面冲击": "RU 盘面冲击",
  "燃料油": "燃料油",
  "乙二醇": "乙二醇",
  "策略研究、回测、参数调优 · vectorbt + DoWhy": "策略研究、回测、参数调优 · vectorbt + DoWhy",
  "添加": "添加",
  "自动保存": "自动保存",
  "关联": "关联",
  "下一步行动": "下一步行动",
  "关联预警 · 关联假设": "关联预警 · 关联假设",
  "假设": "假设",
  "命中率": "命中率",
  "引用此笔记": "引用此笔记",
  "提及的假设": "提及的假设",
  "提及的预警": "提及的预警",
  "假设库": "假设库",
  "交易日志": "交易日志",
  "mature": "成熟",
  "decay": "衰减",
  "suggested": "建议",
  "active weight": "生效权重",
  "candidate": "候选",
  "prior dominant": "先验主导",
  "No resolved calibration buckets are available yet.": "暂无已归因的校准桶。",
  "Some rows are recent resolved-signal candidates and are not active weights yet.":
    "部分行来自近期已归因信号候选，还不是生效权重。",
};

export function I18nProvider({ children }: { children: ReactNode }) {
  const [lang, setLangState] = useState<Language>("zh");

  useEffect(() => {
    const stored = window.localStorage.getItem(STORAGE_KEY);
    if (stored === "zh" || stored === "en") setLangState(stored);
  }, []);

  const setLang = (nextLang: Language) => {
    setLangState(nextLang);
    window.localStorage.setItem(STORAGE_KEY, nextLang);
    document.documentElement.lang = nextLang === "zh" ? "zh-CN" : "en";
  };

  useEffect(() => {
    document.documentElement.lang = lang === "zh" ? "zh-CN" : "en";
  }, [lang]);

  const value = useMemo<I18nContextValue>(
    () => ({
      lang,
      setLang,
      toggleLang: () => setLang(lang === "zh" ? "en" : "zh"),
      text: (source: string) => (lang === "en" ? EN[source] ?? source : ZH[source] ?? source),
    }),
    [lang]
  );

  return <I18nContext.Provider value={value}>{children}</I18nContext.Provider>;
}

export function useI18n() {
  const context = useContext(I18nContext);
  if (!context) throw new Error("useI18n must be used within I18nProvider");
  return context;
}

export function LanguageToggle({ className }: { className?: string }) {
  const { lang, toggleLang } = useI18n();

  return (
    <button
      type="button"
      onClick={toggleLang}
      className={cn(
        "inline-flex h-6 items-center gap-1.5 rounded-sm border border-border-default bg-bg-base px-2 font-mono text-caption text-text-secondary shadow-inner-panel transition-colors hover:border-brand-emerald/40 hover:text-text-primary focus-visible:shadow-focus-ring focus-visible:outline-none",
        className
      )}
      aria-label={lang === "zh" ? "Switch to English" : "切换到中文"}
      title={lang === "zh" ? "Switch to English" : "切换到中文"}
    >
      <Languages className="h-3.5 w-3.5 text-brand-emerald-bright" />
      <span className={lang === "zh" ? "text-text-primary" : ""}>中</span>
      <span className="text-text-disabled">/</span>
      <span className={lang === "en" ? "text-text-primary" : ""}>EN</span>
    </button>
  );
}

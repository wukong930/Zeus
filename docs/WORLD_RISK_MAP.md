...
- [x] Phase B.2.11 瓦片请求调度护栏：前端按筛选、分辨率和近似视口缓存瓦片，缩放/拖拽去抖刷新，并丢弃过期响应避免旧视图覆盖新视图。
- [x] Phase B.2.12 瓦片预算可观测性：增强模式顶部显示瓦片数量、分辨率、缓存条目、负载等级和缓存/网络/刷新状态，避免性能状态只藏在代码里。
- [x] Phase B.2.13 瓦片渲染预算降载：增强模式按轻/标准/密集三档限制天气瓦片、密度点和跨区飞线数量，优先保留强度最高的信息。
- [x] Phase B.2.14 标签避让与缩放密度：地图标签采用固定屏幕尺寸、碰撞检测和负载预算，缩放/拖拽时优先保留选中、高风险和有事件作用域的区域。
- [x] Phase B.2.15 控制条密度与浮层层级：顶部筛选默认折叠为浮动面板，区域详情、错误遮罩、索引和缩放控件使用明确层级。
- [x] Phase B.2.16 浏览器验证流程：新增 `docs/CANVAS_BROWSER_VERIFICATION.md`，固化世界风险地图与因果网络的大画布回归检查点。
- [x] Phase B.2.17 增强模式截图与性能基准：新增自动化 CDP 截图和 DOM / heap / FPS 预算，留档轻量模式、增强模式、筛选态、区域弹窗和 Causal Web。
- [x] Phase B.2 MapLibre + deck.gl：天气栅格、瓦片热力和超大数据量增强。
...
- [x] Phase D.3 风险链阅读动线：区域档案新增“链路总览”，按动量、证据健康、质量门和因果网络统一组织阅读路径。
- [x] Phase D.4 推荐动作入口：区域档案根据质量门、证据健康、动量和作用域推荐复核、补证据、追溯或观察。
- [x] Phase D.5 上下文跳转：推荐动作进入事件智能、新闻或因果网络时携带世界风险地图来源、品种、区域和可用事件作用域。
- [x] Phase E.1 反向联动：事件智能影响链和新闻事件智能链可带着品种、来源、机制、区域和事件作用域打开地图。
- [x] Phase E.2 事件作用域可见化：从事件智能进入地图时，区域档案显示当前事件、命中证据、质量门和是否直接命中该区域。
- [x] Phase E.3 Causal Web 深链：从地图进入因果网络时携带 `event`，因果网络自动聚焦同一事件智能路径。
- [x] Phase E.4 Event Intelligence 深链：从事件详情、影响链、证据和反证入口进入因果网络时保留同一事件作用域。
- [x] Phase E.5 Causal Web 反向入口：从因果网络事件智能节点回到事件智能或世界地图时保留同一事件作用域。
- [x] Phase E.6 News Events 深链：新闻证据页可按事件作用域选中来源新闻，并直接打开同一事件的因果网络或事件智能链。
- [x] Phase E.7 作用域来源收口：因果网络能显示当前深链来自地图、新闻或事件智能，避免同一事件跨页阅读时误判上下文来源。
- [x] Phase D 2D / WebGL 信息阅读增强，3D Globe 暂缓。
- [x] Phase E Event Intelligence Engine 联动：事件源筛选、商品影响机制和证据/反证链。

...
# 2026-05-13 — Causal Web Phase 10.18 事件作用域深链
...
# 2026-05-13 — World Risk Map Phase B.2.11 瓦片请求调度护栏

- 世界风险地图增强层新增瓦片缓存，缓存键由筛选条件、瓦片分辨率和近似视口组成，重复缩放/拖拽不会反复请求同一批瓦片。
- 视口瓦片刷新去抖从散落常量收口为 `WORLD_MAP_TILE_REFRESH_DEBOUNCE_MS`，便于后续根据本地性能统一调参。
- 瓦片请求新增序号保护，旧请求晚返回时不会覆盖用户当前视图，自动刷新仍会强制拉取最新瓦片以保持运行态新鲜度。

# 2026-05-14 — World Risk Map Phase B.2.12 瓦片预算可观测性

- 增强模式顶部控制条新增轻量瓦片预算芯片，展示瓦片总数、粗/中分辨率、轻/标准/密集负载和缓存条目数量。
- 瓦片运行态会区分缓存命中、网络请求和自动强制刷新，方便后续调优 MapLibre/deck.gl 大数据量图层。
- 该指标以窄芯片方式展示，不恢复此前右侧大卡片，避免世界风险地图阅读区域再次变拥挤。

# 2026-05-14 — World Risk Map Phase B.2.13 瓦片渲染预算降载

- 增强模式的瓦片预算不再只是提示，会按轻 / 标准 / 密集三档限制天气瓦片、密度点和跨区飞线渲染数量。
- 降载时优先保留强度最高的瓦片与风险点，保证大数据量场景下先读到最重要的信息。
- 顶部瓦片芯片在标准 / 密集负载下显示“已降载”，让性能护栏对用户可见但不挤占地图主体。
...
# 2026-05-14 — World Risk Map Phase B.2.14 标签避让与缩放密度

- 世界风险地图区域标签改为固定屏幕尺寸，不再跟随地图缩放变大或变小，拖拽/缩放时保持阅读稳定。
- 新增标签优先级与碰撞避让：选中区域、高风险、有事件作用域、运行态证据强的区域优先保留，其余按当前负载预算隐藏。
- 标签安全区域避开顶部控制层和底部边界，减少增强模式下热力、飞线和标签互相遮挡。

# 2026-05-14 — World Risk Map Phase B.2.15 控制层与回归流程
